...
async def run(label, outdir):
    ...
    try:
        ...
        async with websockets.connect(page["webSocketDebuggerUrl"],
            ...
            # Open the panel and let the API fill the numbers in.
            ...
            box = await c.js("JSON.stringify(document.querySelector('.portfolio-panel')"
                             ".getBoundingClientRect().toJSON())")
            b = json.loads(box)
            # Viewport-only. No captureBeyondViewport: the panel is fixed.
            shot = await c.send("Page.captureScreenshot", format="png",
                                clip={"x": max(0, b["x"] - 4), "y": 0,
                                      "width": min(b["width"] + 8, 1440),
                                      "height": 720, "scale": 1})
